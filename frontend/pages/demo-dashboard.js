...
export default function Dashboard() {
  ...
  // Geçici admin bypass - sahte kullanıcı verisi
  const fakeUser = {
  ...
  const fetchChatLogs = async () => {
    try {
      ...
      setChatLogs([
        ...
        {
          message: 'Ürünleriniz hakkında bilgi alabilir miyim?',
          response: 'Tabii! Size ürünlerimiz hakkında detaylı bilgi verebilirim. Hangi ürünle ilgileniyorsunuz?',
  ...
  const regenerateApiKey = async () => {
    try {
      // Sahte API key oluştur
      const newKey = 'demo-api-key-' + Math.random().toString(36).substr(2, 9);
      setNewApiKey(newKey);
...
<script>
  window.ChatbotConfig = {
    ...
    webhookUrl: "${process.env.NEXT_PUBLIC_WIDGET_URL || 'http://localhost:3000'}/webhook/chat/${profile.id}",
    theme: {
      primaryColor: "#3B82F6",
...
<script src="${process.env.NEXT_PUBLIC_WIDGET_URL || 'http://localhost:3000'}/chat-widget.js"></script>`;
  };
  ...
  const logout = () => {
    // Geçici logout - ana sayfaya yönlendir
    router.push('/');
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-4">
            ...
            <div className="flex items-center space-x-4">
              <span className="text-gray-700">Welcome, {fakeUser.email}</span>
              <span className="text-xs bg-green-100 text-green-800 px-2 py-1 rounded">DEMO MODE</span>
              <button
                onClick={logout}
              ...
              <div className="space-y-2">
                ...
                <div>
                  ...
                  <code className="bg-gray-100 px-2 py-1 rounded text-sm block">
                    {process.env.NEXT_PUBLIC_WIDGET_URL || 'http://localhost:3000'}/webhook/chat/{profile?.id}
                  </code>
                </div>
              </div>
            </div>
